    ...
    <aside className="w-80 border-r border-accent-dark bg-panel-dark flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="space-y-2 relative" ref={dropdownRef}>
          <span className="text-[8px] text-gray-600 font-black uppercase tracking-widest">Atmospheric Profile</span>
          ...
          >
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-sm">{getGenreIcon(genre)}</span>
              <span>{genre}</span>
            </div>
            <span className={`material-symbols-outlined text-sm transition-transform duration-300 ${isGenreOpen ? 'rotate-180' : ''}`}>expand_more</span>
          </button>

          {isGenreOpen && (
            <div className="absolute top-full left-0 w-full mt-1 bg-panel-dark border border-white/10 rounded-lg shadow-2xl z-50 overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="py-1">
                {genres.map((g) => (
                  <button
                    key={g}
                  ...
                  >
                    <span className="material-symbols-outlined text-sm">{getGenreIcon(g)}</span>
                    {g}
                  </button>
                ))}
              </div>
